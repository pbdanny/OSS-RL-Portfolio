...
```{r appr_decline_cancel_percent_table, echo=FALSE}
# table of percent
rl_os %>%
    group_by(month, result) %>%
    summarise(finl = n()) %>%
...
```{r top_decline_percent, echo=FALSE}
rl_os %>%
    filter(result == 'D') %>%
    group_by(month, result_description) %>%
    summarise(finl = n()) %>%
...
rm(list = c('month_data', 'month_data_length', 'last_4_month_idxz'))
```
...
```{r cust_age_histogram, echo=FALSE}
month_data <- unique(rl_os$month)
month_data_length <- length(month_data)
last_4_month_idx <- ifelse(month_data_length <= 4, 1, month_data_length - 3)
...
p <- rl_os %>%
    ...
    ggplot(aes(x = cus_age_range, fill = factor(month), 
               color = factor(month))) + 
...
```{r cust_region_table, echo=FALSE}
rl_os %>%
...
Percent of Finalized by region
```{r cust_region_percent_table, echo=FALSE}
# table of percent
rl_os %>%
    group_by(month, region2) %>%
    summarise(finl = n()) %>%
